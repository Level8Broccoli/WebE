...
export function removePlayerFromJoinedGame(
  serverState: ServerState,
  playerId: string
) {
  serverState.games.map(
    (g) => (g.players = g.players.filter((p) => p !== playerId))
  );
}

export function removePlayerFromPlayerList(
...
export function registerExistingPlayer(
  playerIdList: FullPlayer[],
...
export function isCreator(game: Game, playerId: string): boolean {
...
export function gameExists(gameList: Game[], gameId: string): boolean {
  return gameList.find((g) => g.id === gameId) !== undefined;
}

export function freeSpaceInGame(game: Game): boolean {
  return game.players.length < game.config.maxPlayerCount;
}

export function joinGame(game: Game, player: PrivatePlayer) {
...
export function isPlayerCountValid(game: Game): boolean {
  return game.players.length <= game.config.maxPlayerCount;
}
...
export function isLevelValid(
  ...
  const playerLevel = game.state.playerLevels.find(
