...
export const WebSocketPlugin = (socket: Socket) => (store: Store<State>) => {
  ...
  socket.on(
    "updatePlayerList",
    (res: UpdatePlayerListResponse | ErrorResponse) => {
  ...
  socket.on(
    ...
    (res: RegisterExistingPlayerResponse | ErrorResponse) => {
      if ("player" in res) {
        store.commit("updatePlayer", res.player);
        store.commit("updateGames", res.games);
  ...
  socket.on(
    "finishFulfillment",
    (res: UpdateGameBoardResponse | ErrorResponse) => {
      if ("gameId" in res) {
